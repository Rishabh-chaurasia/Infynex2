 ...
 */
export default function DuctScene({ className = '', compact = false }) {
    const reduced = usePrefersReducedMotion();
    const frames = Array.from({ length: compact ? 7 : 10 }, (_, i) => i);
    return (<svg viewBox="0 0 400 300" className={`h-full w-full ${className}`} aria-hidden preserveAspectRatio="xMidYMid slice">
      <defs>
      ...
      {frames.map((i) => {
            ...
            return (<motion.rect key={i} x={x} y={y} width={w} height={h} rx={6 * s} fill="none" stroke="#6fd3ee" strokeOpacity={0.12 + t * 0.4} strokeWidth={0.6 + t * 0.6} initial={{ opacity: 0, scale: 1.1 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: i * 0.08 }} style={{ transformOrigin: '200px 150px' }}/>);
        })}
      {/* Corner guide lines to the vanishing point */}
      {[[10, 15], [390, 15], [10, 285], [390, 285]].map(([x, y], i) => (<motion.line key={i} x1={x} y1={y} x2={200} y2={150} stroke="#6fd3ee" strokeOpacity={0.25} strokeWidth={0.6} initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 1.6, delay: 0.3 }}/>))}
      {/* Seams */}
      {[40, 80, 120, 160].map((s, i) => (<line key={i} x1={10 + s * 0.95} y1={15 + s * 0.68} x2={390 - s * 0.95} y2={15 + s * 0.68} stroke="#ffffff" strokeOpacity={0.05}/>))}

      {/* Scanning beam sweeping the duct */}
      {!reduced && (<motion.rect x="10" width="380" height="22" fill="url(#beam)" animate={{ y: [10, 270, 10] }} transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}/>)}

      {/* Dust particles */}
      {Array.from({ length: compact ? 12 : 22 }, (_, i) => {
            const x = 30 + ((i * 53) % 340), y = 30 + ((i * 97) % 240);
            return (<motion.circle key={i} cx={x} cy={y} r={0.8 + (i % 3) * 0.5} fill="#ffffff" fillOpacity={0.5} animate={reduced ? {} : { y: [0, -8 - (i % 5), 0], x: [0, (i % 2 ? 4 : -4), 0], opacity: [0.2, 0.7, 0.2] }} transition={{ duration: 4 + (i % 4), repeat: Infinity, ease: 'easeInOut', delay: (i % 7) * 0.3 }}/>);
        })}

      {/* Robot travelling into the duct */}
      <motion.g initial={{ scale: 1.05, x: 0, y: 40 }} animate={reduced ? {} : { scale: [1.05, 0.55, 1.05], y: [40, 8, 40] }} transition={{ duration: 14, repeat: Infinity, ease: 'easeInOut' }} style={{ transformOrigin: '200px 150px' }}>
