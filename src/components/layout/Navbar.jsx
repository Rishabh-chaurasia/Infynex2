...
export default function Navbar() {
    ...
    return (<>
      <motion.header initial={false} className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-500 ${solid ? 'border-teal-500/15 bg-white/90 backdrop-blur-2xl' : 'border-transparent bg-transparent'} ${scrolled || mega ? 'shadow-[0_14px_45px_-22px_rgba(11,21,51,0.3)]' : ''}`} onMouseLeave={() => setMega(false)}>
        <div className="container-x flex items-center justify-between transition-all duration-500" style={{ height: scrolled ? 'calc(var(--nav-h) * .8)' : 'var(--nav-h)' }}>
          ...
          <nav className="hidden items-center gap-9 lg:flex" aria-label="Primary">
            {navLinks.map((l) => l.label === 'Services' ? (<div key={l.to} className="relative" onMouseEnter={() => setMega(true)}>
                ...
                </div>) : (<NavLink key={l.to} to={l.to} className={linkCls} end={l.to === '/'} onMouseEnter={() => setMega(false)}>
                  {({ isActive }) => (<span className="relative">
                      {l.label}
          ...
          <button className="relative z-10 flex h-11 w-11 items-center justify-center rounded-full bg-navy-900 text-white lg:hidden" onClick={() => setOpen((v) => !v)} aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open}>
        ...
        {/* Mega menu */}
        <AnimatePresence>
          {mega && (<motion.div initial={{ opacity: 0, y: -12, clipPath: 'inset(0 0 100% 0)' }} animate={{ opacity: 1, y: 0, clipPath: 'inset(0 0 0 0)' }} exit={{ opacity: 0, y: -8, clipPath: 'inset(0 0 100% 0)' }} transition={{ duration: 0.5, ease: EASE }} className="absolute inset-x-0 top-full hidden border-t border-navy-800/5 lg:block">
              <div className="bg-ivory-50/95 backdrop-blur-2xl">
                <div className="container-x grid grid-cols-[1.2fr_3fr] gap-10 py-10">
                  ...
                  <motion.ul className="mega-services-grid grid gap-2" initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.035, delayChildren: 0.1 } } }}>
                    {navServices.map(({ service: s, label }) => {
                const Icon = s.icon;
                return (<motion.li key={s.slug} variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: EASE } } }}>
                          <NavLink to={s.path} className={({ isActive }) => `group flex items-start gap-3 rounded-2xl p-3 transition-colors duration-300 hover:bg-navy-900/[0.04] ${isActive ? 'bg-navy-900/[0.05]' : ''}`}>
      ...
      {/* Mobile menu */}
      <AnimatePresence>
        {open && (<motion.div className="fixed inset-0 z-40 lg:hidden" initial="closed" animate="open" exit="closed">
            <motion.div className="absolute inset-0 bg-navy-950" variants={{ closed: { clipPath: 'circle(0% at calc(100% - 44px) 44px)' }, open: { clipPath: 'circle(150% at calc(100% - 44px) 44px)' } }} transition={{ duration: 0.7, ease: EASE }}/>
            <div aria-hidden className="absolute inset-0 grid-lines-dark opacity-50"/>
            <motion.nav className="relative flex h-full flex-col overflow-y-auto px-7 pb-10 pt-28" variants={{ closed: {}, open: { transition: { staggerChildren: 0.07, delayChildren: 0.25 } } }} aria-label="Mobile">
              {navLinks.map((l) => (<motion.div key={l.to} variants={{ closed: { opacity: 0, y: 30, x: -10 }, open: { opacity: 1, y: 0, x: 0, transition: { duration: 0.6, ease: EASE } } }} className="border-b border-white/10">
                  {l.label === 'Services' ? (<>
                      <button className="flex w-full items-center justify-between py-5 font-display text-3xl font-semibold text-white" onClick={() => setMobileServices((v) => !v)} aria-expanded={mobileServices}>
                        Services
                        <ChevronDown className={`h-6 w-6 transition-transform duration-300 ${mobileServices ? 'rotate-180' : ''}`}/>
                      </button>
                      <AnimatePresence>
                        {mobileServices && (<motion.ul initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.45, ease: EASE }} className="overflow-hidden pb-4">
                            <li><Link to="/services" className="block py-2 text-teal-300">All services</Link></li>
                            {navServices.map(({ service: s, label }) => (<li key={s.slug}><Link to={s.path} className="block py-2 text-white/75">{label}</Link></li>))}
                          </motion.ul>)}
                      </AnimatePresence>
