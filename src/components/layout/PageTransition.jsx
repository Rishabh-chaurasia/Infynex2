...
export default function PageTransition({ children }) {
    return (<motion.div initial="initial" animate="enter" exit="exit" className="relative">
      <motion.div className="route-transition-content" variants={{
      ...
      <motion.div aria-hidden className="route-transition-curtain pointer-events-none fixed inset-0 z-[80] bg-navy-900" style={{ transformOrigin: 'bottom' }} variants={{
            initial: { scaleY: 0 },
            enter: { scaleY: 0 },
            exit: { scaleY: 1, transition: { duration: 0.5, ease: EASE_IN_OUT } },
        }}/>
      {/* Enter curtains — lift away toward the top */}
      <motion.div aria-hidden className="route-transition-curtain pointer-events-none fixed inset-0 z-[81] bg-navy-900" style={{ transformOrigin: 'top' }} variants={{
            initial: { scaleY: 1 },
            enter: { scaleY: 0, transition: { duration: 0.8, ease: EASE_IN_OUT, delay: 0.05 } },
            exit: { scaleY: 0 },
        }}/>
      <motion.div aria-hidden className="route-transition-curtain pointer-events-none fixed inset-0 z-[80] bg-teal-500" style={{ transformOrigin: 'top' }} variants={{
            initial: { scaleY: 1 },
