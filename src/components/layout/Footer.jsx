...
export default function Footer() {
    return (<footer className="site-footer relative overflow-hidden border-t border-navy-800/10 bg-ivory-100">
      ...
      <motion.div className="container-x relative grid gap-8 py-10 sm:grid-cols-2 sm:gap-12 sm:py-14 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr] lg:py-14 2xl:gap-14 2xl:py-20" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-10% 0px' }} variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.12 } } }}>
...
function FooterLink({ to, children }) {
    return (<Link to={to} className="group inline-flex items-center gap-2 text-sm text-ink-600 transition-colors hover:text-navy-900">
      <span className="h-px w-0 bg-teal-500 transition-all duration-500 group-hover:w-4"/>
      {children}
    </Link>);
}
function Social({ href, label, brand, children }) {
    ...
    return (<a href={href} target="_blank" rel="noreferrer" aria-label={label} data-cursor="hover" className={`flex h-10 w-10 items-center justify-center rounded-full border transition-all duration-500 hover:-translate-y-1 ${brandClass}`}>
      {children}
    </a>);
}
function LinkedInIcon() {
    return (<svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor" aria-hidden>
...
function WhatsAppIcon() {
    return (<svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="currentColor" aria-hidden>
