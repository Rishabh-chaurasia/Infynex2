...
export default function Infrastructure() {
    ...
    return (<section ref={ref} className="relative bg-ivory-50 pb-8">
      ...
      <div className="container-x relative -mt-10 md:-mt-24">
        <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ duration: 1, ease: EASE }} className="ml-auto max-w-2xl rounded-[2rem] border border-navy-800/10 bg-white p-8 shadow-[0_40px_100px_-40px_rgba(11,21,51,0.35)] md:p-10">
          <p className="eyebrow text-teal-600">Infrastructure services</p>
          <ul className="mt-6 divide-y divide-navy-800/10">
            {links.map((l, i) => (<motion.li key={l.to} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={viewportOnce} transition={{ delay: 0.2 + i * 0.08, duration: 0.7, ease: EASE }}>
