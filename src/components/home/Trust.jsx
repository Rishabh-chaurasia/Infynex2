...
const reasons = [
    { icon: ShieldCheck, title: 'End-to-End Ownership', text: 'One accountable team manages every stage from planning to delivery.' },
    { icon: Headphones, title: 'Expert Support On Demand', text: 'Prompt remote and on-site assistance keeps operations running smoothly.' },
    { icon: ClipboardCheck, title: 'Documented Services', text: 'Clear audits and service reports provide complete visibility and traceability.' },
    { icon: Layers3, title: 'Scalable Service Models', text: 'Flexible service models adapt as your operational requirements evolve.' },
];
/** Compact reasons section designed to fit in one desktop viewport. */
export function WhyInfynex() {
    return (<section className="why-infynex-section relative overflow-hidden bg-ivory-50 pb-12 pt-0 md:pb-16 md:pt-0">
      <div aria-hidden className="absolute inset-0 grid-lines opacity-35"/>
      <div className="container-x relative">
        <SectionHeading eyebrow="Why Infynex" title="Your Trusted Partner for Seamless Business Operations" size="md" className="mb-5 [&_.eyebrow]:!text-sm md:[&_.eyebrow]:!text-base"/>
        <div className="grid overflow-hidden rounded-3xl bg-navy-950 text-white shadow-[0_20px_60px_rgba(10,27,61,0.12)] md:grid-cols-[0.8fr_1.2fr]">
          <ImageReveal src={images.meeting} alt="Infynex team working with a client" className="h-44 md:h-full md:min-h-48" direction="left"/>
          <div className="flex flex-col justify-center px-6 py-6 md:px-8">
            <p className="font-display text-xs font-bold uppercase tracking-[0.2em] text-teal-300">Dedicated Service Support</p>
            <h3 className="mt-3 max-w-2xl font-display text-2xl font-semibold leading-tight md:text-3xl">Reliable Support. Whenever You Need It.</h3>
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-white/70">Please tell us what you need. Our team will listen, explain the options clearly and help you at every step.</p>
          </div>
        </div>
...
export function Clients() {
    ...
    useEffect(() => {
        const animation = trackRef.current?.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(-50%)' }], { duration: 24000, iterations: Infinity, easing: 'linear' });
        return () => animation?.cancel();
    }, []);
    return (<section className="relative overflow-hidden bg-ivory-100 py-16 text-navy-950 md:py-20">
      ...
      <div className="relative mt-10 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)]">
        <div ref={trackRef} className="flex w-max will-change-transform">
          {[0, 1].map((copy) => (<div key={copy} className="flex shrink-0 gap-4 pr-4" aria-hidden={copy === 1 ? 'true' : undefined}>
              {[...clients, ...clients].map((c, i) => (<div key={`${copy}-${c.name}-${i}`} className="flex h-28 min-w-[17rem] items-center justify-center rounded-2xl border border-navy-800/10 bg-white px-4 shadow-sm">
...
export function BlogPreview() {
    ...
    useEffect(() => {
        const animation = trackRef.current?.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(-50%)' }], { duration: 36000, iterations: Infinity, easing: 'linear' });
        animationRef.current = animation ?? null;
        return () => animation?.cancel();
    }, []);
    return (<section className="home-blog-preview relative overflow-hidden bg-navy-950 py-12 text-white md:py-16">
