...
export default function Capabilities() {
    ...
    return (<section ref={ref} className="relative overflow-hidden bg-navy-950 py-14 text-white md:py-20">
      ...
      <div className="container-x relative grid items-center gap-16 lg:grid-cols-[1fr_1.15fr]">
        <div>
          <SectionHeading dark eyebrow="Capabilities" title="Four capability areas, connected by design." text="Every Infynex service sits inside one of four capability areas. Because the same team spans all of them, hand-offs between infrastructure, maintenance, support and facilities work happen inside the company — not between vendors."/>
          <div className="mt-12 space-y-4">
            {caps.map((c, i) => {
            const Icon = c.icon;
            return (<motion.div key={c.title} initial={{ opacity: 0, x: i % 2 ? 60 : -60, rotateY: i % 2 ? 20 : -20 }} whileInView={{ opacity: 1, x: 0, rotateY: 0 }} viewport={{ once: true, margin: '-10% 0px' }} transition={{ duration: 1, ease: EASE, delay: i * 0.1 }} className="group flex items-start gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-5 transition-all duration-500 hover:border-teal-400/40 hover:bg-white/[0.06]" data-cursor="hover">
        ...
        <motion.div style={{ y: rise }} className="relative perspective">
          <motion.div style={{ rotateY: rotate, transformStyle: 'preserve-3d' }} className="relative">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.8, duration: 0.8 }} className="absolute bottom-6 left-6 rounded-2xl border border-teal-500/40 bg-white/95 p-4 text-navy-950 shadow-[0_16px_40px_rgba(11,21,51,.28)] backdrop-blur" style={{ transform: 'translateZ(60px)' }}>
