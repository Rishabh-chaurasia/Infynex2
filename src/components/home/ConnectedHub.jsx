...
export default function ConnectedHub() {
    ...
    return (<section id="capabilities" className="connected-hub connected-hub-dark relative overflow-hidden bg-[radial-gradient(circle_at_50%_45%,rgba(10,80,99,.36),transparent_30%),linear-gradient(145deg,#041521,#08283a_58%,#061c2a)] pb-10 pt-10 text-white lg:min-h-[calc(100svh-80px)]">
      ...
      <div className="container-x relative">
        ...
        <div className="relative mx-auto my-6 grid max-w-[700px] grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-3 lg:my-2 lg:block lg:h-[330px]">
          <svg className="absolute inset-0 hidden h-full w-full overflow-visible lg:block" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
            {points.map(([x, y], index) => (<line key={index} x1="50" y1="48" x2={50 + (x - 50) * .88} y2={48 + (y - 48) * .88} stroke={active === index ? accents[index] : '#45cce0aa'} strokeWidth={active === index ? 3 : 1.5} strokeDasharray={active === index ? undefined : '4 3'} vectorEffect="non-scaling-stroke" className="transition-all duration-300"/>))}
          </svg>
          ...
          {hubServices.map((service, index) => {
            ...
            return (<Link key={service.slug} to={service.path} onMouseEnter={() => setActive(index)} onFocus={() => setActive(index)} onClick={() => setActive(index)} style={{ '--node-accent': accents[index], left: `${x}%`, top: `${y}%` }} className={`hub-service-node hub-service-node-minimal group relative z-[4] flex min-h-[5.5rem] flex-col items-center justify-center rounded-xl px-2 text-center transition-all duration-300 lg:absolute lg:min-h-[54px] lg:w-36 lg:-translate-x-1/2 lg:-translate-y-1/2 lg:rounded-none ${isActive ? 'lg:scale-110' : 'hover:scale-105'}`}>
        ...
        <Link key={`mobile-${selected.slug}`} to={selected.path} style={{ '--active-accent': accent }} className="hub-detail mx-auto grid w-full max-w-[460px] items-center gap-3 rounded-2xl border border-white/10 bg-white/[.035] px-4 py-3 shadow-[inset_4px_0_var(--active-accent)] transition-colors hover:bg-white/[.07] lg:hidden">
          <div>
