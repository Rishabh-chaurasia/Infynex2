...
export function ServicesIntro() {
    return (<section className="container-x py-24 md:py-32">
      <div className="grid gap-12 md:grid-cols-[1fr_1.1fr] md:gap-20">
        <div>
          <SectionHeading eyebrow="What we do" title="Complete services. One accountable partner." text="Most organisations juggle a maintenance vendor, a hardware supplier, a network integrator, a support desk and a facilities contractor. Infynex brings these under one relationship so that ownership is never in question."/>
        ...
        <motion.ul className="divide-y divide-navy-800/10 border-y border-navy-800/10" initial="hidden" whileInView="visible" viewport={viewportOnce} variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.06 } } }}>
          {services.map((s, i) => {
            const Icon = s.icon;
            return (<motion.li key={s.slug} variants={{ hidden: { opacity: 0, x: 30 }, visible: { opacity: 1, x: 0, transition: { duration: 0.8, ease: EASE } } }}>
...
export function ServiceShowcase() {
    ...
    useEffect(() => {
        const animation = trackRef.current?.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(-50%)' }], { duration: 56000, iterations: Infinity, easing: 'linear' });
        animationRef.current = animation ?? null;
        return () => animation?.cancel();
    }, []);
    return (<section className="service-showcase-section relative overflow-hidden bg-navy-950 pb-12 pt-16 text-white md:pb-14 md:pt-20">
              ...
              {featured.map((service, index) => {
                ...
                <div className="absolute inset-0 flex flex-col justify-between p-7 sm:p-9">
                  ...
                  <div className="flex min-h-[16.5rem] flex-col justify-end">
                    ...
                    <h3 className="service-card-heading mt-2 flex min-h-[5.5rem] items-end font-display text-3xl font-semibold leading-tight drop-shadow-[0_2px_12px_rgba(0,0,0,.55)] xl:text-4xl" style={{ '--service-heading-color': headingColors[index] }}>{service.title}</h3>
