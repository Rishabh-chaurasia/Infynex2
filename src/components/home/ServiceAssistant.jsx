...
function answerFor(query) {
    ...
    if (value.includes('solar') || value.includes('esg'))
        return 'We provide site surveys, solar system design, installation and ongoing maintenance.';
    if (value.includes('amc') || value.includes('maintenance'))
        return 'Our AMC service covers regular checks, preventive maintenance, repairs and clear service reports.';
    if (value.includes('support') || value.includes('helpdesk') || value.includes('it'))
        return 'We provide remote helpdesk support and on-site engineers for IT issues, installations and repairs.';
    if (value.includes('hardware'))
        return 'We help select, supply, install, upgrade and repair computers, servers and network equipment.';
    if (value.includes('duct') || value.includes('robot'))
        return 'Our camera-guided robots inspect and clean HVAC ducts with clear before-and-after documentation.';
    if (value.includes('cloud'))
        return 'We can set up, migrate and manage cloud servers, data, access and backups.';
    if (value.includes('vehicle') || value.includes('transport') || value.includes('fleet'))
        return 'We help companies procure the number and type of new vehicles they need, with clear pricing, delivery and ongoing support.';
    if (value.includes('tele') || value.includes('call'))
        return 'Our trained team can manage customer calls, follow-ups and agreed communication processes.';
    if (value.includes('contact') || value.includes('team') || value.includes('price') || value.includes('quote'))
        return `You can call us at ${site.contact.phone}, WhatsApp us, or send an enquiry from the contact page.`;
...
export default function ServiceAssistant() {
    ...
    const [messages, setMessages] = useState(['Hello. We are here to help with your business services.']);
    const ask = (question) => {
        if (!question.trim())
            return;
    ...
    return (<div className="service-assistant fixed bottom-3 right-3 z-[70] sm:bottom-7 sm:right-7">
      <AnimatePresence>
      ...
      <motion.button onClick={() => setOpen((value) => !value)} whileHover={{ scale: 1.035, y: -2 }} whileTap={{ scale: .97 }} aria-label="Open Infynex service assistant" className="group relative ml-auto flex h-11 w-11 items-center justify-center overflow-visible rounded-full bg-gradient-to-br from-teal-500 via-sky-500 to-[#486ee8] text-white shadow-[0_16px_42px_rgba(26,157,195,.42)] sm:h-14 sm:w-14">
