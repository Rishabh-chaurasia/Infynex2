...
import { ArrowUpRight, Headphones, ShieldCheck, Waypoints } from 'lucide-react';
...
export default function Hero() {
    return (<section className="source-hero relative isolate flex min-h-[620px] items-center overflow-hidden bg-[#041521] text-white sm:min-h-[680px] lg:min-h-[700px]">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 hidden lg:block">
        ...
        {signals.map((signal, index) => (<motion.div key={signal.label} className={`absolute flex items-center gap-2.5 text-white ${signal.className}`} animate={{ y: [0, -5, 0] }} transition={{ duration: 3, repeat: Infinity, delay: index * -.45 }}>
            ...
            <span className="grid border-l border-teal-200 bg-[#071e2db8] px-2.5 py-1.5 backdrop-blur-md"><b className="source-signal-label tracking-[.16em]">{signal.label}</b><small className="source-signal-value tracking-[.11em] text-teal-200">{signal.value}</small></span>
          </motion.div>))}
      </div>

      <div className="source-hero-content container-x relative z-10 pb-7 pt-28 md:pt-32">
        <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65, delay: .1, ease: EASE }} className="eyebrow mb-4 block text-[length:var(--step--1)] text-teal-300 sm:mb-7">Infynex Technologies</motion.span>
        <motion.h1 initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .9, delay: .15, ease: EASE }} className="max-w-[72ch] font-display text-[length:var(--source-hero-heading-size)] font-medium leading-[1.07] tracking-[-.035em] md:max-w-[62%]">Powering your business<br /><span className="source-hero-accent">with reliable, results-driven solutions.</span></motion.h1>
        <motion.p initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8, delay: .3, ease: EASE }} className="mt-4 max-w-[760px] text-[length:var(--step-0)] leading-[1.65] text-[#baf5ff] [text-shadow:0_0_18px_rgba(66,220,245,.22)] sm:mt-6 sm:leading-[1.75] md:max-w-[61%]">From robust IT infrastructure, hardware maintenance, AMC services and technical support to facilities management, energy, mobility, robotic duct cleaning and integrated operational solutions, Infynex Technologies enhances efficiency, strengthens security and ensures seamless business continuity.</motion.p>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8, delay: .42, ease: EASE }} className="my-5 grid gap-2.5 sm:my-8 sm:flex sm:flex-wrap sm:items-center sm:gap-4 md:mb-12"><Link to="/services" className="group inline-flex min-h-12 w-full items-center justify-center gap-3 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 px-5 font-display text-sm font-bold text-white shadow-[0_14px_38px_-16px_rgba(42,183,221,.9)] transition duration-300 hover:-translate-y-1 sm:min-h-14 sm:w-auto sm:px-8 sm:text-base hover:shadow-[0_18px_44px_-14px_rgba(42,183,221,1)]">Explore Our Services <ArrowUpRight className="h-5 w-5 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1"/></Link><Link to="/contact" className="group inline-flex min-h-12 w-full items-center justify-center gap-3 rounded-full border border-white/20 bg-gradient-to-r from-[#526ee8] to-[#815fd7] px-5 font-display text-sm font-bold text-white shadow-[0_14px_38px_-16px_rgba(93,91,220,.9)] transition duration-300 hover:-translate-y-1 sm:min-h-14 sm:w-auto sm:px-8 sm:text-base hover:shadow-[0_18px_44px_-14px_rgba(114,91,220,1)]">Talk to Our Team <ArrowUpRight className="h-5 w-5 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1"/></Link></motion.div>
