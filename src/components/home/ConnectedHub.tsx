...
export default function ConnectedHub() {
  ...
  const detailSide = selectedX <= 50 ? 'md:right-[calc(100%+1rem)]' : 'md:left-[calc(100%+1rem)]'
  ...
  return (
    <section id="capabilities" className="connected-hub connected-hub-dark relative min-h-[calc(100svh-80px)] overflow-hidden bg-[radial-gradient(circle_at_50%_45%,rgba(10,80,99,.36),transparent_30%),linear-gradient(145deg,#041521,#08283a_58%,#061c2a)] pb-10 pt-8 text-white md:pt-10">
      ...
      <div className="container-x relative">
        ...
        <div className="relative mx-auto my-2 grid max-w-[700px] grid-cols-2 gap-3 sm:grid-cols-3 md:block md:h-[330px]">
          <svg className="absolute inset-0 hidden h-full w-full overflow-visible md:block" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
          ...
          <div aria-hidden className="absolute left-1/2 top-[48%] hidden h-20 w-32 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-[1.4rem] border border-teal-200/30 bg-[#071e2d]/90 shadow-[0_0_60px_rgba(74,213,230,.24)] backdrop-blur md:grid">
          ...
          {hubServices.map((service, index) => {
            ...
            return (
              <Link
                ...
                className={`hub-service-node hub-service-node-minimal group relative z-[4] flex min-h-24 flex-col items-center justify-center px-2 text-center transition-all duration-300 md:absolute md:min-h-[54px] md:w-36 md:-translate-x-1/2 md:-translate-y-1/2 ${isActive ? 'scale-110' : 'hover:scale-105'}`}
              >
        ...
        <Link key={`mobile-${selected.slug}`} to={selected.path} style={{ '--active-accent': accent } as CSSProperties} className="hub-detail grid max-w-[460px] items-center gap-3 rounded-2xl border border-white/10 bg-white/[.035] px-4 py-3 shadow-[inset_4px_0_var(--active-accent)] transition-colors hover:bg-white/[.07] md:hidden">
