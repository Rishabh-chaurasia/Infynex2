...
export default function ServiceAssistant() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[70] sm:bottom-7 sm:right-7">
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 20, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 15, scale: .96 }} className="mb-4 flex h-[32rem] max-h-[70vh] w-[calc(100vw-2.5rem)] flex-col overflow-hidden rounded-3xl border border-white/15 bg-navy-950 text-white shadow-[0_24px_80px_rgba(4,14,37,.38)] sm:w-96">
      ...
      <motion.button onClick={() => setOpen((value) => !value)} whileHover={{ scale: 1.035, y: -2 }} whileTap={{ scale: .97 }} aria-label="Open Infynex service assistant" className="group relative ml-auto flex h-16 items-center gap-3 overflow-visible rounded-full bg-gradient-to-br from-teal-500 via-sky-500 to-[#486ee8] px-3 text-white shadow-[0_16px_42px_rgba(26,157,195,.42)] sm:px-4">
