...
export function RoboticHighlight() {
    ...
    return (<section className="relative overflow-hidden bg-navy-950 py-24 text-white md:py-32">
      ...
      <div className="container-x grid items-center gap-14 lg:grid-cols-[1.15fr_1fr]">
        <motion.div initial={{ opacity: 0, scale: 0.94, rotateY: -12 }} whileInView={{ opacity: 1, scale: 1, rotateY: 0 }} viewport={viewportOnce} transition={{ duration: 1.2, ease: EASE }} className="perspective relative aspect-[4/3] overflow-hidden rounded-[2rem] border border-white/10 shadow-[0_60px_120px_-40px_rgba(111,211,238,0.25)]">
        ...
        <div>
          ...
          <motion.ul variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.1 } } }} initial="hidden" whileInView="visible" viewport={viewportOnce} className="mt-8 grid grid-cols-2 gap-3">
            {s.process.map((p) => (<motion.li key={p.title} variants={fadeUp} className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 font-display text-sm font-semibold">{p.title}</motion.li>))}
          </motion.ul>
...
export function SolarHighlight() {
    ...
    return (<section ref={ref} className="relative overflow-hidden bg-[#fff8ec] py-14 md:py-20">
      ...
      <div className="container-x grid items-center gap-14 lg:grid-cols-[1fr_1.2fr]">
        <div className="order-2 lg:order-1">
          ...
          <motion.div variants={fadeUp} custom={4} initial="hidden" whileInView="visible" viewport={viewportOnce} className="mt-8 flex flex-wrap gap-2">
            {s.highlights.slice(0, 4).map((h) => (<span key={h} className="rounded-full border border-[#f5b544]/50 bg-white px-4 py-2 text-sm text-navy-900">{h}</span>))}
          </motion.div>
...
export function B2BHighlight() {
    ...
    return (<section className="home-b2b-section container-x pb-12 pt-0 md:pb-14 md:pt-0">
      <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
        ...
        <div className="relative flex rounded-full border border-navy-800/15 bg-white p-1" role="tablist">
          {['b2b', 'b2c'].map((m) => (<button key={m} role="tab" aria-selected={mode === m} onClick={() => setMode(m)} className={`relative z-10 rounded-full px-6 py-2.5 font-display text-sm font-semibold uppercase tracking-wider transition-colors duration-300 ${mode === m ? 'text-white' : 'text-navy-900'}`}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={mode} initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 30 }} transition={{ duration: 0.5, ease: EASE }}>
            ...
            <ul className="mt-3 flex flex-wrap gap-1.5">
              {content.points.map((p, i) => (<motion.li key={p} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 + i * 0.08 }} className={`rounded-full px-2.5 py-1 text-[.68rem] ${mode === 'b2b' ? 'bg-white/10' : 'bg-white'}`}>{p}</motion.li>))}
            </ul>
