...
export default function SectionHeading({
  eyebrow, title, text, align = 'left', dark = false, className = '', size = 'lg',
}: Props) {
  const sizes = {
  ...
  return (
    <div className={`${align === 'center' ? 'mx-auto text-center' : ''} max-w-3xl ${className}`}>
      ...
      {text && (
        <motion.p
          variants={fadeUp} custom={3} initial="hidden" whileInView="visible" viewport={viewportOnce}
          className={`mt-6 text-lg leading-relaxed ${dark ? 'text-white/70' : 'text-ink-600'}`}
        >
          {text}
        </motion.p>
      )}
