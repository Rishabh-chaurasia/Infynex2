...
export default function TiltCard({ children, className = '', max = 8, glare = true }) {
    ...
    return (<motion.div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} style={{ rotateX: desktop ? rx : 0, rotateY: desktop ? ry : 0, transformStyle: 'preserve-3d' }} className={`group relative ${className}`} data-cursor="hover">
      {children}
      {glare && desktop && (<motion.div aria-hidden className="pointer-events-none absolute inset-0 rounded-[inherit] opacity-0 transition-opacity duration-500 group-hover:opacity-100" style={{ background: glareBg }}/>)}
    </motion.div>);
}
