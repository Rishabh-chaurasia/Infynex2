...
export default function ServiceCard({ service, index = 0, variant = 'tall' }: Props) {
  ...
  return (
    <motion.div
      ...
      className="service-directory-card perspective"
    >
      <TiltCard className={`service-card-shell rounded-3xl ${variant === 'tall' ? 'aspect-[4/5]' : 'aspect-[16/10]'}`}>
        <Link
          to={service.path}
          className="service-card-link relative block h-full w-full overflow-hidden rounded-3xl bg-navy-900 text-white shadow-[0_30px_80px_-30px_rgba(11,21,51,0.45)]"
          aria-label={`View ${service.title}`}
        >
          <SmartImage
            src={service.hero}
            alt=""
            className="service-card-image absolute inset-0 h-full w-full object-cover opacity-70 transition-transform duration-[1400ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-110"
          />
