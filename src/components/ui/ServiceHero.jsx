 ...
 */
export default function ServiceHero({ service, layout = 'full', visual, imageClassName = '', tone = 'light' }) {
    ...
    const Crumbs = (<motion.nav initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className={`mb-6 flex flex-wrap items-center gap-2 text-xs sm:mb-8 ${tone === 'dark' ? 'text-white/60' : 'text-ink-400'}`} aria-label="Breadcrumb">
    ...
    const Copy = (<motion.div style={{ y: textY, opacity: fade }} className="service-hero-copy relative z-10">
      {Crumbs}
      <motion.p initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3, ease: EASE }} className="eyebrow mb-5 flex items-center gap-3" style={{ color: service.accent }}>
        <span className="h-px w-10" style={{ background: service.accent }}/>
        {service.eyebrow}
      </motion.p>
      <AnimatedText as="h1" trigger="mount" delay={0.35} text={service.title} className={`text-balance font-display text-4xl font-semibold leading-[1.02] sm:text-5xl md:text-6xl lg:text-7xl ${tone === 'dark' ? 'text-white' : 'text-navy-900'}`}/>
      <motion.p initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.8, ease: EASE }} className={`mt-6 max-w-xl text-lg md:text-xl ${tone === 'dark' ? 'text-white/70' : 'text-ink-600'}`}>
        {service.tagline}
      </motion.p>
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 1, ease: EASE }} className="service-hero-actions mt-8 flex flex-wrap gap-3 sm:mt-10 sm:gap-4">
    ...
    if (layout === 'split') {
        return (<section ref={ref} className={`service-hero relative overflow-hidden ${tone === 'dark' ? 'bg-navy-950 text-white' : 'bg-ivory-50'}`}>
        ...
        <div className="container-x relative grid min-h-[92vh] items-center gap-10 pb-14 pt-28 md:grid-cols-2 md:pb-20 md:pt-40">
          {Copy}
          <motion.div initial={{ clipPath: 'inset(0 0 0 100%)', opacity: 0 }} animate={{ clipPath: 'inset(0 0 0 0)', opacity: 1 }} transition={{ duration: 1.4, ease: EASE, delay: 0.4 }} className="relative aspect-[4/5] overflow-hidden rounded-[2rem] md:aspect-[5/6]">
    ...
    if (layout === 'offset') {
        return (<section ref={ref} className={`service-hero relative overflow-hidden ${tone === 'dark' ? 'bg-navy-950 text-white' : 'bg-ivory-100'}`}>
        <motion.div initial={{ clipPath: 'inset(0 0 100% 0)' }} animate={{ clipPath: 'inset(0 0 0 0)' }} transition={{ duration: 1.5, ease: EASE, delay: 0.2 }} className="service-hero-offset-media absolute right-0 top-0 h-full w-full md:w-[62%]">
          <motion.div style={{ y: imgY, scale: imgScale }} className="absolute inset-[-10%]">
            <SmartImage src={service.hero} alt="" className={`h-full w-full object-cover ${imageClassName}`}/>
          </motion.div>
    ...
    return (<section ref={ref} className={`service-hero relative min-h-[100svh] overflow-hidden ${tone === 'dark' ? 'bg-navy-950 text-white' : 'bg-ivory-50 text-navy-950'}`}>
      <motion.div style={{ y: imgY, scale: imgScale }} className="absolute inset-[-10%]">
        <SmartImage src={service.hero} alt="" className={`h-full w-full object-cover ${imageClassName}`}/>
      </motion.div>
