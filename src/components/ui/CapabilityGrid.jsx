 ...
 */
export default function CapabilityGrid({ items, dark = false, accent = '#1a9dc3', layout = 'grid' }) {
    const wrap = {
        grid: 'grid gap-6 sm:grid-cols-2',
        stagger: 'grid gap-6 sm:grid-cols-2 [&>*:nth-child(even)]:sm:translate-y-10',
        row: 'auto-grid',
    }[layout];
    return (<div className={`${wrap} perspective`}>
      {items.map((c, i) => (<motion.div key={c.title} initial={{ opacity: 0, y: 40, rotateX: 10 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} viewport={{ once: true, margin: '-8% 0px' }} transition={{ duration: 0.9, ease: EASE, delay: i * 0.1 }}>
