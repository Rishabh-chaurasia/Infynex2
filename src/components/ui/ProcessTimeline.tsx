 ...
 */
export default function ProcessTimeline({ steps, dark = false, orientation = 'horizontal', accent = '#1a9dc3' }: Props) {
      ...
      <div ref={ref} className="relative pl-10 md:pl-14">
        ...
        <ol className="space-y-12">
          {steps.map((s, i) => (
            ...
            >
              <span
                className={`absolute -left-10 top-1 flex h-6 w-6 items-center justify-center rounded-full border-2 md:-left-14 md:h-10 md:w-10 ${dark ? 'border-navy-900 bg-navy-800' : 'border-ivory-50 bg-white'} font-display text-xs font-semibold`}
                style={{ boxShadow: `0 0 0 2px ${accent}`, color: accent }}
              >
                {i + 1}
              </span>
