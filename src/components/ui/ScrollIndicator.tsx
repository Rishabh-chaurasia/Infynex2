...
export default function ScrollIndicator({ dark = true }: { dark?: boolean }) {
  ...
  return (
    <motion.div
      ...
      className={`absolute bottom-8 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-3 md:flex ${c}`}
      aria-hidden
    >
      <span className="eyebrow text-[0.6rem]">Scroll</span>
      <span className={`relative h-10 w-6 rounded-full border ${c}`}>
        <motion.span
          className={`absolute left-1/2 top-2 h-1.5 w-1.5 -translate-x-1/2 rounded-full ${dark ? 'bg-teal-300' : 'bg-teal-500'}`}
