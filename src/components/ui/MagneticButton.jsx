 ...
 */
export default function MagneticButton({ children, to, href, onClick, variant = 'primary', className = '', icon = true, type = 'button', disabled, }) {
    ...
    const inner = (<span className="relative z-10 flex items-center gap-2">
      <span>{children}</span>
      {icon && (<span className="relative h-4 w-4 overflow-hidden">
    ...
    const sheen = (<span aria-hidden className="pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform duration-700 ease-out group-hover:translate-x-full"/>);
    return (<motion.div ref={ref} style={{ x: sx, y: sy }} onMouseMove={onMove} onMouseLeave={onLeave} className="inline-block" whileTap={{ scale: 0.96 }} data-cursor="hover">
      {to ? (<Link to={to} className={cls}>{sheen}{inner}</Link>) : href ? (<a href={href} target="_blank" rel="noreferrer" className={cls}>{sheen}{inner}</a>) : (<button type={type} onClick={onClick} disabled={disabled} className={cls}>{sheen}{inner}</button>)}
    </motion.div>);
}
