 ...
 */
export default function ServiceCard({ service, index = 0, variant = 'tall' }) {
    ...
    return (<motion.div variants={{
        ...
        }} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-8% 0px' }} className="service-directory-card perspective">
      <TiltCard className={`service-card-shell rounded-3xl ${variant === 'tall' ? 'aspect-[4/5]' : 'aspect-[16/10]'}`}>
        <Link to={service.path} className="service-card-link relative block h-full w-full overflow-hidden rounded-3xl bg-navy-900 text-white shadow-[0_30px_80px_-30px_rgba(11,21,51,0.45)]" aria-label={`View ${service.title}`}>
          <SmartImage src={service.hero} alt="" className="service-card-image absolute inset-0 h-full w-full object-cover opacity-70 transition-transform duration-[1400ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-110"/>
          <div className="service-card-overlay absolute inset-0 bg-gradient-to-t from-navy-950 via-navy-950/55 to-navy-950/10"/>
          ...
          <div className="service-card-content relative flex h-full flex-col justify-between p-6 md:p-7" style={{ transform: 'translateZ(30px)' }}>
            ...
            <div className="service-card-copy">
              ...
              <div className="service-card-description grid transition-[grid-template-rows] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] [grid-template-rows:0fr] group-hover:[grid-template-rows:1fr]">
