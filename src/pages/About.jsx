...
export default function About() {
    ...
    return (<>
      ...
      <section className="relative overflow-hidden bg-navy-950 pb-0 pt-20 text-white md:pb-0 md:pt-24">
        ...
        ].map((t, i) => (<motion.p key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: 0.3 + i * 0.15, duration: 0.9, ease: EASE }} className="border-t border-white/15 pt-6 text-white/70">{t}</motion.p>))}
          </div>
        </div>
      </section>

      {/* Values */}
      <section className="about-promises-section container-x pb-14 pt-10 md:pb-16 md:pt-12">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-12">
          ...
          <ol className="grid gap-3 sm:grid-cols-2">
            {values.map((v, i) => (<motion.li key={v.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ duration: 0.7, ease: EASE }} className="group flex min-h-[260px] flex-col rounded-2xl border border-navy-800/10 bg-ivory-50 p-7 lg:min-h-[280px] lg:p-8" data-cursor="hover">
