...
function RouteLines() {
  ...
  return (
    <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 1440 600" preserveAspectRatio="xMidYMid slice" aria-hidden>
...
function TripCard({ from, to, when, i }: { from: string; to: string; when: string; i: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.12, duration: 0.9, ease: EASE }}>
    ...
    <>
        ...
        <div className="grid items-center gap-14 lg:grid-cols-2">
          <div>
            ...
            <motion.div variants={fadeUp} custom={4} initial="hidden" whileInView="visible" viewport={viewportOnce} className="mt-10 grid grid-cols-2 gap-4">
              {[{ icon: CalendarClock, t: 'Scheduled & on-demand' }, { icon: Users, t: 'Staff & visitor transport' }, { icon: MapPin, t: 'Route coordination' }, { icon: Receipt, t: 'Trip records & billing' }].map(({ icon: Icon, t }) => (
      ...
      <section ref={ref} className="overflow-hidden bg-navy-950 py-16 text-white">
        <motion.div style={{ x: driftL }} className="flex gap-6 pl-6">
          {[s.gallery[1], s.gallery[2], s.gallery[0], s.gallery[1]].map((img, i) => (
            <div key={i} className="w-[60vw] shrink-0 md:w-[32vw]"><ImageReveal src={img} alt="" className="aspect-[16/10] rounded-3xl" direction={i % 2 ? 'left' : 'right'} hoverZoom={false} /></div>
          ))}
        </motion.div>
