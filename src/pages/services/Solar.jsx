...
function SunRays() {
    return (<div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      <motion.div className="absolute -right-32 -top-32 h-[34rem] w-[34rem] rounded-full" style={{ background: 'radial-gradient(circle, rgba(245,181,68,0.55), rgba(245,181,68,0.1) 45%, transparent 70%)' }} animate={{ scale: [1, 1.08, 1], opacity: [0.8, 1, 0.8] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}/>
      ...
      <section ref={ref} className="relative overflow-hidden bg-navy-950 py-24 text-white md:py-32">
        ...
        <div className="container-x">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {solutions.map((sol, i) => {
            const Icon = sol.icon;
            return (<motion.div key={sol.title} style={{ y: i % 2 ? p2 : p1 }} initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.1, duration: 0.9, ease: EASE }}>
      ...
      <section className="bg-[#fff8ec] py-24 md:py-32">
        <div className="container-x">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {s.capabilities.map((c, i) => (<motion.div key={c.title} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.1, duration: 0.9, ease: EASE }} className="rounded-3xl border border-[#f5b544]/40 bg-white p-7">
