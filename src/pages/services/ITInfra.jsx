...
function LayeredNetwork({ active, setActive }) {
    return (<svg viewBox="0 0 100 100" className="h-full w-full" aria-hidden>
      <defs>
      ...
      {layers.map((l, i) => (<motion.rect key={l.name} x="4" y={l.y - 9} width="92" height="18" rx="3" fill="#6fd3ee" animate={{ fillOpacity: active === i ? 0.09 : 0.02 }} transition={{ duration: 0.4 }} onMouseEnter={() => setActive(i)} style={{ cursor: 'pointer' }}/>))}
      {/* inter-layer links */}
      {layers.slice(0, -1).map((l, i) => {
            const next = layers[i + 1];
            return l.nodes.flatMap((x) => next.nodes.filter((nx) => Math.abs(nx - x) < 24).map((nx, j) => (<g key={`${i}-${x}-${nx}`}>
              <motion.line x1={x} y1={l.y} x2={nx} y2={next.y} stroke="#6fd3ee" strokeWidth="0.25" animate={{ strokeOpacity: active === i || active === i + 1 ? 0.7 : 0.2 }} initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 1.2, delay: i * 0.2 + j * 0.05 }}/>
              <motion.circle r="0.5" fill="#ffffff" filter="url(#ng)" animate={{ cx: [x, nx], cy: [l.y, next.y], opacity: [0, 1, 0] }} transition={{ duration: 2 + (j % 3) * 0.6, repeat: Infinity, delay: (x + j) % 3, ease: 'easeInOut' }}/>
            </g>)));
        })}
      {layers.map((l, i) => l.nodes.map((x, j) => (<g key={`${i}-${j}`} onMouseEnter={() => setActive(i)} style={{ cursor: 'pointer' }}>
          <motion.circle cx={x} cy={l.y} r={i === 0 ? 2.4 : 1.8} fill="#0b1533" stroke="#6fd3ee" strokeWidth="0.4" initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 + i * 0.15 + j * 0.05, type: 'spring' }} animate={{ stroke: active === i ? '#ffffff' : '#6fd3ee' }} style={{ transformOrigin: `${x}px ${l.y}px` }}/>
          <motion.circle cx={x} cy={l.y} r={1} fill="#6fd3ee" animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 2 + j * 0.3, repeat: Infinity }}/>
        </g>)))}
      {layers.map((l, i) => (<text key={l.name} x="5" y={l.y - 5.5} fontSize="2.6" fontWeight="700" fontFamily="Sora, sans-serif" fill="#0b1533" fillOpacity={active === i ? 1 : 0.72}>{l.name.toUpperCase()}</text>))}
    </svg>);
}
/**
...
export default function ITInfra() {
    ...
    return (<>
      <ServiceHero service={s} layout="full" visual={<div className="absolute inset-0 opacity-50 mix-blend-screen"><NetworkVisual seed={21} nodes={20}/><Particles count={30} links={false}/></div>}/>
