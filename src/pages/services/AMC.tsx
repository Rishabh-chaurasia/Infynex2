    ...
    <>
      ...
      <section className="container-x py-24 md:py-32">
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
          <SectionHeading eyebrow="Why AMC matters" title="Reactive or planned? Compare the two." />
          <div className="flex rounded-full border border-navy-800/15 bg-white p-1">
            {(['reactive', 'planned'] as const).map((m) => (
              <button key={m} onClick={() => setMode(m)} className={`relative rounded-full px-6 py-2.5 font-display text-sm font-semibold capitalize transition-colors ${mode === m ? 'text-white' : 'text-navy-900'}`}>
                {mode === m && <motion.span layoutId="amc-pill" className="absolute inset-0 -z-10 rounded-full bg-navy-900" transition={{ type: 'spring', stiffness: 300, damping: 30 }} />}
                {m}
              </button>
            ))}
          </div>
