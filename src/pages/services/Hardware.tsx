...
function FloatingParts() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 hidden md:block" aria-hidden>
      {parts.map((p, i) => (
        <motion.div key={p.label} className="absolute" style={{ left: p.l, top: p.t }}
          initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1.2 + i * 0.15, duration: 0.8, ease: EASE }}>
          <motion.div animate={{ y: [0, -12, 0], rotate: [0, 3, 0] }} transition={{ duration: 6 + i, repeat: Infinity, ease: 'easeInOut', delay: p.d }}
            className="glass-dark rounded-xl border border-white/20 px-3 py-2 font-display text-[0.65rem] tracking-[0.25em] text-white">
          ...
          <div className="mt-14 grid gap-10 lg:grid-cols-[1fr_1.1fr]">
            <ul className="space-y-2">
              {categories.map((c, i) => {
                const Icon = c.icon
                return (
                  <motion.li key={c.title} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.06, duration: 0.7, ease: EASE }}>
                    <button
                      onMouseEnter={() => setActive(i)} onFocus={() => setActive(i)} onClick={() => setActive(i)}
                      className={`flex w-full items-center gap-4 rounded-2xl border p-4 text-left transition-all duration-500 ${active === i ? 'border-teal-400/50 bg-white/[0.08] pl-6' : 'border-white/10 hover:bg-white/[0.04]'}`}
                    >
