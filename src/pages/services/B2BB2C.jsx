...
export default function B2BB2C() {
    ...
    return (<>
      ...
      <LayoutGroup>
        <motion.section layout animate={{ backgroundColor: dark ? '#0b1533' : '#f4f6fa' }} transition={{ duration: 0.8, ease: EASE }} className="relative overflow-hidden py-24 md:py-32">
          ...
          <div className="container-x relative">
            ...
            return (<button key={m} role="tab" aria-selected={mode === m} onClick={() => setMode(m)} className={`relative z-10 flex items-center gap-2 rounded-full px-6 py-3 font-display text-sm font-semibold transition-colors duration-300 ${mode === m ? (dark ? 'text-navy-950' : 'text-white') : dark ? 'text-white/70' : 'text-navy-900'}`}>
              ...
              <AnimatePresence mode="wait">
                <motion.div key={mode} initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 40 }} transition={{ duration: 0.55, ease: EASE }}>
                  ...
                  <ul className="mt-3 flex flex-wrap gap-2">
                    {c.services.map((sv, i) => (<motion.li key={sv} initial={{ opacity: 0, scale: 0.8, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.05, duration: 0.5, ease: EASE }} className={`rounded-full px-4 py-2 text-sm ${dark ? 'bg-white/10 text-white' : 'bg-white text-navy-900 ring-1 ring-navy-800/10'}`}>{sv}</motion.li>))}
                  </ul>
                </motion.div>
              </AnimatePresence>
            ...
            return (<motion.div key={mode + card.title} layout initial={{ opacity: 0, y: 40, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.95 }} transition={{ duration: 0.6, ease: EASE, delay: i * 0.08 }}>
