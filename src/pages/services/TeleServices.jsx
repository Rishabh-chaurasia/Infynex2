...
function SignalWaves() {
    ...
    return (<div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      <div className="absolute left-[30%] top-[38%]">
        {[0, 1, 2, 3].map((i) => (<motion.span key={i} className="absolute -left-4 -top-4 h-8 w-8 rounded-full border border-teal-300/60" animate={reduced ? {} : { scale: [1, 14], opacity: [0.7, 0] }} transition={{ duration: 6, repeat: Infinity, delay: i * 1.5, ease: 'easeOut' }}/>))}
      </div>
      <svg className="absolute bottom-0 left-0 h-40 w-full opacity-70" viewBox="0 0 1200 160" preserveAspectRatio="none">
        <motion.path d="M0 80 Q 50 20 100 80 T 200 80 T 300 80 T 400 80 T 500 80 T 600 80 T 700 80 T 800 80 T 900 80 T 1000 80 T 1100 80 T 1200 80" fill="none" stroke="#6fd3ee" strokeWidth="1.5" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 3, delay: 0.8 }}/>
        <motion.path d="M0 80 Q 50 140 100 80 T 200 80 T 300 80 T 400 80 T 500 80 T 600 80 T 700 80 T 800 80 T 900 80 T 1000 80 T 1100 80 T 1200 80" fill="none" stroke="#ffffff" strokeOpacity="0.4" strokeWidth="1" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 3, delay: 1.2 }}/>
      </svg>
    </div>);
}
...
function Workflow() {
    ...
    return (<div className="relative">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-9 hidden h-4 md:block" aria-hidden>
        {[0, 1, 2].map((i) => (<motion.span key={i} className="absolute top-1 h-2 w-2 rounded-full bg-teal-400 shadow-[0_0_12px_#2ab7dd]" animate={{ left: ['2%', '98%'], opacity: [0, 1, 1, 0] }} transition={{ duration: 5, repeat: Infinity, delay: i * 1.7, ease: 'easeInOut' }}/>))}
      </div>
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        {steps.map((st, i) => {
            const Icon = st.icon;
            return (<motion.div key={st.label} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.15, duration: 0.9, ease: EASE }} className="relative">
      ...
      <section className="relative overflow-hidden bg-navy-950 py-24 text-white md:py-32">
        ...
        <div className="container-x">
          ...
          <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {s.capabilities.map((c, i) => (<motion.div key={c.title} initial={{ opacity: 0, y: 40, rotateY: -15 }} whileInView={{ opacity: 1, y: 0, rotateY: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.1, duration: 0.9, ease: EASE }} className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-7 transition-colors duration-500 hover:bg-white/[0.08]" data-cursor="hover">
