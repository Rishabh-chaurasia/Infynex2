  ...
  return (
    <div className="pointer-events-none absolute inset-0 hidden md:block" aria-hidden>
      {tools.map((t, i) => {
        ...
        return (
          <motion.div key={t.label} className="absolute left-1/2 top-1/2" style={{ x: Math.cos(angle) * 220 - 24, y: Math.sin(angle) * 150 - 24 }}
            initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1 + i * 0.12, type: 'spring', stiffness: 160, damping: 14 }}>
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 4 + i * 0.6, repeat: Infinity, ease: 'easeInOut' }}
              className="glass-dark flex h-12 w-12 items-center justify-center rounded-2xl border border-white/20 text-white">
          ...
          <div className="lg:pt-6">
            ...
            <motion.ul variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.06 } } }} initial="hidden" whileInView="visible" viewport={viewportOnce} className="mt-8 flex flex-wrap gap-2">
              {tools.map((t) => {
                const Icon = t.icon
                return (
                  <motion.li key={t.label} variants={{ hidden: { opacity: 0, scale: 0.8 }, visible: { opacity: 1, scale: 1 } }} className="flex items-center gap-2 rounded-full border border-navy-800/15 bg-white px-4 py-2 text-sm text-navy-900">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-12">
            ...
            <div className="grid content-center gap-4 md:col-span-4">
              {s.capabilities.slice(0, 3).map((c, i) => (
                <motion.div key={c.title} initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.12, duration: 0.8, ease: EASE }} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 transition-colors duration-500 hover:bg-white/[0.08]" data-cursor="hover">
