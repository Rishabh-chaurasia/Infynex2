...
function ScrollProcess() {
    ...
    if (!desktop) {
        ...
        <div className="container-x">
          ...
          <ol className="mt-10 space-y-4">
            {s.process.map((st, i) => (<motion.li key={st.title} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.1, duration: 0.7, ease: EASE }} className="flex gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-5">
    ...
    return (<section ref={ref} className="robotic-process relative h-[320dvh] bg-navy-950 text-white">
      <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden">
        ...
        <div className="container-x relative">
          ...
          <div className="robotic-track relative mt-14 h-40 rounded-3xl border-2 border-teal-500 bg-white/90 shadow-[0_18px_55px_rgba(26,157,195,.2)]">
            ...
            {Array.from({ length: 9 }, (_, i) => (<span key={i} className="absolute top-4 bottom-4 w-px bg-white/5" style={{ left: `${10 + i * 10}%` }}/>))}
            {s.process.map((st, i) => (<Stage key={st.title} index={i} progress={p} title={st.title}/>))}
...
function Stage({ index, progress, title }) {
    ...
    return (<motion.div style={{ left, scale, opacity }} className="absolute top-1/2 -translate-y-1/2">
...
function StageText({ index, progress, title, text }) {
    const at = index / 3;
...
export default function RoboticDuctCleaning() {
    ...
    return (<>
      ...
      <section className="bg-ivory-100 py-24 md:py-32">
        <div className="container-x">
          ...
          <div className="mt-16 grid gap-6 md:grid-cols-3">
            {[s.gallery[1], s.gallery[2], s.gallery[3]].map((img, i) => (<motion.div key={i} initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.15, duration: 1, ease: EASE }} className={i === 1 ? 'md:-mt-12' : ''}>
        ...
        <div className="grid gap-14 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div>
            ...
            <ul className="mt-10 space-y-3">
              {equipment.map((e, i) => {
            const Icon = e.icon;
            return (<motion.li key={e.title} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.1, duration: 0.8, ease: EASE }} className="group flex gap-4 rounded-2xl border border-navy-800/10 bg-white p-4 transition-all duration-500 hover:border-teal-500/40 hover:pl-6" data-cursor="hover">
      ...
      <section className="robotic-benefits relative overflow-hidden bg-navy-950 py-24 text-white md:py-32">
        ...
        ].map(([t, d], i) => (<motion.div key={t} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.12, duration: 0.9, ease: EASE }} className="robotic-benefit-card rounded-3xl border-2 border-teal-500/30 bg-white/95 p-8 shadow-[0_18px_50px_rgba(11,21,51,.12)]">
