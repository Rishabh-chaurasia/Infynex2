      ...
      <section className="container-x py-24 md:py-32">
        <div className="grid items-center gap-14 lg:grid-cols-[1fr_1.1fr]">
          <div className="relative">
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={viewportOnce} transition={{ duration: 1, ease: EASE, delay: 0.4 }} className="absolute -bottom-8 -right-6 hidden w-[46%] md:block">
          ...
          <div>
            ...
            <motion.div variants={fadeUp} custom={4} initial="hidden" whileInView="visible" viewport={viewportOnce} className="mt-10 grid grid-cols-2 gap-4">
              {[{ icon: CalendarCheck, t: 'Scheduled visits' }, { icon: ClipboardList, t: 'Service reports' }, { icon: Repeat, t: 'Full-term coverage' }, { icon: AlertTriangle, t: 'Fault attendance' }].map(({ icon: Icon, t }) => (<div key={t} className="flex items-center gap-3 rounded-2xl border border-navy-800/10 bg-white p-4" data-cursor="hover">
      ...
      <section className="bg-ivory-100 py-24 md:py-32">
        <div className="container-x grid gap-14 lg:grid-cols-2 lg:items-center">
          ...
          <div className="relative">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: 0.6, duration: 0.8 }} className="absolute bottom-6 left-6 right-6 rounded-2xl bg-white/90 p-5 backdrop-blur">
      ...
      <section className="container-x py-24 md:py-32">
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
          ...
          <div className="flex rounded-full border-2 border-teal-500/35 bg-white p-1.5 shadow-[0_10px_30px_rgba(26,157,195,.16)]">
            {['reactive', 'planned'].map((m) => (<button key={m} onClick={() => setMode(m)} className={`relative isolate overflow-hidden rounded-full px-7 py-3 font-display text-sm font-bold capitalize transition-colors ${mode === m ? 'text-white' : 'text-navy-900'}`}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={mode} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5, ease: EASE }} className={`mt-12 grid gap-6 rounded-[2rem] p-8 md:grid-cols-3 md:p-12 ${mode === 'planned' ? 'bg-navy-900 text-white' : 'bg-ivory-100 text-navy-900'}`}>
            {(mode === 'planned'
            ...
            : [['Unplanned downtime', 'Work stops until someone is available to attend.'], ['Unpredictable spend', 'Each call-out and part is a separate, unbudgeted cost.'], ['No history', 'Nobody has a record of what was done to which machine.']]).map(([t, d], i) => (<motion.div key={t} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.1 }} className={`rounded-2xl p-6 ${mode === 'planned' ? 'bg-white/[0.06]' : 'bg-white'}`}>
