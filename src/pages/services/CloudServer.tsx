  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      {Array.from({ length: 14 }, (_, i) => (
        <motion.span key={i} className="absolute bottom-0 w-px bg-gradient-to-t from-teal-300/0 via-teal-300/80 to-teal-300/0"
          style={{ left: `${6 + i * 6.5}%`, height: `${30 + (i % 4) * 15}%` }}
          animate={{ y: ['110%', '-110%'] }} transition={{ duration: 4 + (i % 5), repeat: Infinity, ease: 'linear', delay: (i * 0.7) % 4 }} />
      ))}
        ...
        <div className="container-x">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {planes.map((pl, i) => {
              const Icon = pl.icon
              return (
                <motion.div key={pl.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewportOnce} transition={{ delay: i * 0.1, duration: 0.8, ease: EASE }} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        ...
        <div className="container-x grid items-center gap-16 lg:grid-cols-2">
          <div>
            ...
            <ul className="mt-10 space-y-3">
              {planes.map((pl, i) => {
                const Icon = pl.icon
                return (
                  <PlaneRow key={pl.name} icon={Icon} name={pl.name} text={pl.text} index={i} progress={spread} />
                )
              })}
            </ul>
          </div>
          <div className="perspective relative h-[32rem]">
            <motion.div style={{ rotateX: rotX, rotateZ: -20, transformStyle: 'preserve-3d' }} className="absolute inset-0 flex items-center justify-center">
              {planes.map((pl, i) => (
                <Plane key={pl.name} index={i} progress={spread} label={pl.name} />
              ))}
            </motion.div>
          </div>
        </div>
...
function PlaneRow({ icon: Icon, name, text, index, progress }: { icon: typeof Cpu; name: string; text: string; index: number; progress: ReturnType<typeof useTransform<number, number>> }) {
  const start = index * 0.22
  const opacity = useTransform(progress, [start, start + 0.2], [0.35, 1])
  const x = useTransform(progress, [start, start + 0.2], [-10, 0])
  return (
    <motion.li style={{ opacity, x }} className="flex items-start gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
...
function Plane({ index, progress, label }: { index: number; progress: ReturnType<typeof useTransform<number, number>>; label: string }) {
      ...
      <section className="container-x py-24 md:py-32">
        <div className="grid gap-14 lg:grid-cols-[1.1fr_1fr] lg:items-center">
          ...
          <div className="relative">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={viewportOnce} transition={{ delay: 0.5, duration: 0.8, ease: EASE }}
              className="absolute -left-6 bottom-10 hidden w-[48%] md:block">
